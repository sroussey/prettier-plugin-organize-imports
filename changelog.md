Version `6.0.0` moves off `@typescript/typescript6` and organizes imports with the project's own TypeScript 7 again. TypeScript 7 has no in-process language service to call, but its native compiler still implements organize-imports and exposes it over LSP, so the plugin now starts `tsc --lsp --stdio` from the resolved `typescript`, requests the `source.organizeImports` code action and applies the edits. `typescript` is a **required peer dependency** again, declared as `>=7.1.0-dev`; the plugin itself has no runtime dependencies. Both options survive the move: `organizeImportsSkipDestructiveCodeActions` selects the `source.sortImports` action instead, and `organizeImportsTypeOrder` is passed to the server as a preference.

**Vue support is gone.** It worked by decorating the in-process `LanguageService` with Volar, and there is no such object in TypeScript 7 — organizing happens in a separate server process that knows nothing about Volar. The `vue` parser is no longer registered and the optional `vue-tsc` peer dependency has been removed; Prettier still formats `.vue` files, it just leaves their imports alone. Stay on version `5` if you need it.

Version `5.0.0` is the first release of the `@sroussey/prettier-plugin-organize-imports` fork. The plugin now **brings its own TypeScript**: it depends on `@typescript/typescript6` and organizes imports with that language service, so `typescript` is no longer a peer dependency and the plugin works whatever version a project has, or none at all. TypeScript 7.0 dropped the programmatic language service API this plugin is built on, so following the project's own TypeScript was no longer possible. The release also ships as **ESM only**, which drops support for Prettier 2 (Prettier 3 loads plugins with a dynamic `import()`, so consuming projects can still be CommonJS) and lets the plugin use Prettier's `prettier/plugins/*` entry points instead of the deprecated `prettier/parser-*` ones. Internally the plugin is now written in TypeScript and built, bundled and tested with [Bun](https://bun.sh); the published package is a single `dist/index.js` with type declarations alongside it. The plugin's options and sorting behaviour are otherwise unchanged.
